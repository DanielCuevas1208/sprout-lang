...
type Compiler struct {
	...
	fns []fnState
	// loops is the stack of enclosing loops.
	loops []loopInfo
...
func Compile(file *source.File, prog *ast.Program) (p *code.Program, err error) {
	...
	if len(prog.Stmts) > 0 {
		pos = prog.Stmts[len(prog.Stmts)-1].End()
	}
	c.b().Add(code.OpReturn, pos)
	main := c.finishFn()
...
func (c *Compiler) enterFn(name string, params []*ast.Param, body *ast.Block) uint16 {
	...
	pos := source.Pos{Line: 1, Column: 1}
	if len(body.Stmts) > 0 {
		pos = body.Stmts[len(body.Stmts)-1].End()
	}
	c.b().Add(code.OpReturn, pos)
	fn := c.finishFn()
	c.popFn()
...
func (c *Compiler) compileStmts(stmts []ast.Stmt) {
	for _, s := range stmts {
		if fn, ok := s.(*ast.FnStmt); ok {
			c.declare(fn.Name.Name, false, fn.Name.Position)
		}
	}
	for _, s := range stmts {
		c.compileStmt(s)
...
func (c *Compiler) emitBreak(pos source.Pos) {
	if len(c.loops) == 0 {
		c.failf(pos, "internal error: break outside a loop")
	}
	top := &c.loops[len(c.loops)-1]
...
func (c *Compiler) emitContinue(pos source.Pos) {
	if len(c.loops) == 0 {
		c.failf(pos, "internal error: continue outside a loop")
	}
	top := c.loops[len(c.loops)-1]
	c.emitEnvCleanup(top.envDepth, pos)
