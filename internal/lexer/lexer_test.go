...
func TestKeywords(t *testing.T) {
	src := "let const fn if elif else for in while return break continue true false nil and or not import as\n"
	toks := lex(src)
	got := kinds(toks)
	want := []token.Kind{
		...
		token.OR, token.NOT, token.IMPORT, token.AS, token.NEWLINE, token.EOF,
	}
	if len(got) != len(want) {
		t.Fatalf("token count: got %d, want %d\n%v", len(got), len(want), got)
	}
